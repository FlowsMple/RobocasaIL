        ...
        if self.optimizer is None:
            ...
            if self.args.policy_head_lr is not None:
                ...
                mllm_param = [
                    ...
                    if policy_heads_str not in name
                ]
                policy_heads_param = [
                    name
                    for name, _ in opt_model.named_parameters()
                    if policy_heads_str in name
                ]
            ...
            if self.local_rank == 0:
                ...
                model_num_params = sum(
                    1 if p.requires_grad else 0 for p in opt_model.parameters()
                )
                assert (
                    sum_up == model_num_params
                ), f"The total parameters of Optimier Groups {sum_up} must equal the total number of parameters of Model {model_num_params}"
