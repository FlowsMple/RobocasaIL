...
def load_model_for_eval(
    model_path, model_base, device_map="cuda:0", policy_config=None
):
    ...
    if "lora" in model_path.lower() and model_base is not None:
        model, tokenizer = load_merge_lora_weights(
            model_path=model_path, model_base=model_base, kwargs=kwargs
        )
        ...
        if policy_config["save_model"]:
            ...
            for k, v in model.named_parameters():
                if any(skip_param in k.lower() for skip_param in skip_params):
                    head_param[k] = v
            torch.save(
                head_param, os.path.join(model_path, "merge_weights/head_params.bin")
            )
