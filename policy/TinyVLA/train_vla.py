...
def train_bc(train_dataset=None, model=None, config=None, tokenizer=None):
    ...
    data_collator = DataCollatorForSupervisedDataset(
        computed_type=compute_dtype, tokenizer=tokenizer
    )
    ...
    trainer = VLATrainer(
        model=model, tokenizer=tokenizer, args=config["training_args"], **data_module
    )
...
def main(all_config, model_config):
    ...
    model, data_args = model_load_utils.load_model(
        config=all_config, vla_config=model_config, rank0_print=rank0_print
    )

    rank0_print(
        f"{RED} Using {all_config['model_args'].model_name_or_path} as VLA backbone {RESET}"
    )
    ...
    train_bc(
        train_dataset=train_dataset, model=model, config=all_config, tokenizer=tokenizer
    )
    # save dataset stats
    stats_path = os.path.join(
        all_config["training_args"].output_dir, f"dataset_stats.pkl"
...
if __name__ == "__main__":
    ...
    if config["training_args"].resume_from_checkpoint is not None:
        rank0_print(
            f"{RED}Resuming Training from {config['training_args'].resume_from_checkpoint}............{RESET}"
        )
