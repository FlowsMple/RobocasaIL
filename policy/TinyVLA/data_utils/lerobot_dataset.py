...
class TransformedDataset(Dataset[T_co]):
    def __init__(
        ...
        if (
            "diffusion" in self.policy_class.lower()
            or "scale_dp" in self.policy_class.lower()
        ):
            self.augment_images = True
        else:
    ...
    def __getitem__(self, index: SupportsIndex) -> T_co:
        ...
        language_raw = self._dataset.meta.episodes[data["episode_index"]][
            "language_dict"
        ]["language_raw"]
        if self.data_args.use_reasoning:
            none_counter = 0
            for k in ["substep_reasonings", "reason"]:
                vals = self._dataset.meta.episodes[data["episode_index"]][
                    "language_dict"
                ][k]
                if vals is not None:
            ...
            if none_counter == 2:
                self.rank0_print(
                    f"{RED} In {self._dataset.meta.repo_id}-{index}:{k} is None {RESET}"
                )
        ...
        for cam_name in self.camera_names:
            ...
            if image.shape[0] == 3:  # If image is in (channels, height, width)
                image = np.transpose(
                    image, (1, 2, 0)
                )  # Now it's (height, width, channels
        ...
        # normalize to [-1, 1]
        action_data = (
            (action_data - norm_stats["action_min"])
            / (norm_stats["action_max"] - norm_stats["action_min"])
        ) * 2 - 1
        ...
        return self.vla_data_post_process.forward_process(
            sample, use_reasoning=self.data_args.use_reasoning
        )
...
def get_norm_stats(dataset_list):
    ...
    for dataset in tqdm(dataset_list):
        ...
        mean_array = (
            mean_tensor.cpu().numpy() if mean_tensor.is_cuda else mean_tensor.numpy()
        )
        std_array = (
            std_tensor.cpu().numpy() if std_tensor.is_cuda else std_tensor.numpy()
        )
        ...
        action_mean = (
            action_mean.cpu().numpy() if action_mean.is_cuda else action_mean.numpy()
        )
        action_std = (
            action_std.cpu().numpy() if action_std.is_cuda else action_std.numpy()
        )
        action_min = (
            action_min.cpu().numpy() if action_min.is_cuda else action_min.numpy()
        )
        action_max = (
            action_max.cpu().numpy() if action_max.is_cuda else action_max.numpy()
        )
        ...
        length_list.append(
            len(dataset)
        )  # This is a single number, representing the number of samples
    ...
    state_weighted_variance = (
        np.sum(
            (
                (length_array[:, None] - 1) * std_array**2
                + (length_array[:, None] - 1) * mean_array**2
            ),
            axis=0,
        )
        / np.sum(length_array)
    ...
    action_weighted_mean = np.sum(action_mean.T * length_array, axis=1) / np.sum(
        length_array
    )
    action_weighted_variance = (
        np.sum(
            (
                (length_array[:, None] - 1) * action_std**2
                + (length_array[:, None] - 1) * action_mean**2
            ),
            axis=0,
        )
        / np.sum(length_array)
        - action_weighted_mean**2
    )
    ...
    eps = 0.0001
    stats = {
        "action_mean": action_weighted_mean,
        "action_std": action_weighted_std,
...
def create_dataset(
    repo_id, chunk_size, home_lerobot=None, local_debug=False
) -> Dataset:
    ...
    if local_debug:
        ...
        dataset = LeRobotDataset(
            repo_id, delta_timestamps=delta_timestamps, local_files_only=True
        )
    return dataset
    ...
    **kwargs,
    ...
    for repo_id in repo_id_list:
        dataset = create_dataset(
            repo_id,
            chunk_size,
            home_lerobot=config["data_args"].home_lerobot,
            local_debug=config["training_args"].local_debug,
        )
    ...
    robot = (
        ...
        or ("aloha" in config["training_args"].output_dir)
        else "franka"
    )
