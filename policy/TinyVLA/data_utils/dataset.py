...
class EpisodicDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.rank0_print(
            f"{RED}policy class: {self.policy_class}; augument: {self.augment_images}{RESET}"
        )
        a = self.__getitem__(0)  # initialize self.is_sim and self.transformations
        self.rank0_print(
            f"The robot is {RED} {self.robot} {RESET} | The camera views: {RED} {self.camera_names}{RESET}"
        )
        self.is_sim = False
