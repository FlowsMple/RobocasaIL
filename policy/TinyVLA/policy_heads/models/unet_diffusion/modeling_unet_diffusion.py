...
class Conv1dBlock(nn.Module):
    ...
    def __init__(self, inp_channels, out_channels, kernel_size, n_groups=8):
        ...
        self.block = nn.Sequential(
            ...
            nn.GroupNorm(n_groups, out_channels, affine=False),
            nn.Mish(),
        )
...
class ConditionalUnet1D(PreTrainedModel):
    ...
    def __init__(self, config: UnetDiffusionPolicyConfig):
        ...
        self.norm_after_pool = nn.LayerNorm(
            config.global_cond_dim, elementwise_affine=False, eps=1e-6
        )
        self.combine = nn.Linear(
            config.global_cond_dim + config.state_dim, config.global_cond_dim
        )
