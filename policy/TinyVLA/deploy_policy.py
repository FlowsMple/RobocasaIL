...
class TinyVLA:
    def __init__(self, policy_config, camera_names):
        ...
        model_base = policy_config[
            "model_base"
        ]  # if policy_config["enable_lore"] else None
        ...
        self.config = AutoConfig.from_pretrained(
            model_path, trust_remote_code=False, attn_implementation="default"
        )
    ...
    def get_action(self, obs=None):
        ...
        actions = (
            self.policy.sample_action(**batch).detach().cpu().to(torch.float32).numpy()
        )
