...
def process_one_episode(
    input_path, output_path, episode_idx, instruction_dir, resize_size=256
):
    with h5py.File(input_path, "r") as f:
        ...
        head = decode_and_resize_images(
            f["observation/head_camera/rgb"][()], size=resize_size
        )
        left = decode_and_resize_images(
            f["observation/left_camera/rgb"][()], size=resize_size
        )
        right = decode_and_resize_images(
            f["observation/right_camera/rgb"][()], size=resize_size
        )
        front = decode_and_resize_images(
            f["observation/front_camera/rgb"][()], size=resize_size
        )
    ...
    with h5py.File(output_path, "w") as f:
        f.create_dataset(
            ...
            data=head,
            dtype="uint8",
            chunks=(1, resize_size, resize_size, 3),
        )
        f.create_dataset(
            "left_wrist_image",
            data=left,
        ...
        f.create_dataset(
            "seen", data=np.array(seen_list, dtype=h5py.string_dtype(encoding="utf-8"))
        )
        f.create_dataset(
            "unseen",
            data=np.array(unseen_list, dtype=h5py.string_dtype(encoding="utf-8")),
        )
...
def main(args):
    ...
    for split_name, split_eps in [("train", train_eps), ("val", val_eps)]:
        ...
        for i, ep in enumerate(tqdm(split_eps, desc=f"Processing {split_name}")):
            ...
            try:
                process_one_episode(
                    ep, out_path, i, instruction_dir, resize_size=resize_size
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_path", type=str, required=True, help="Path to RoboTwin hdf5 files"
    )
    parser.add_argument(
        "--out_base_dir",
        type=str,
    ...
    parser.add_argument(
        "--img_resize_size", type=int, default=256, help="Final size for RGB images"
    )
