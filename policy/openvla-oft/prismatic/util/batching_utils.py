...
class SplitModalitySampler(Sampler):
    ...
    def get_modality_and_length_grouped_indices(
        ...
        mm_batches = [
            mm_reindexed[i : i + g_bsz] for i in range(0, len(mm_reindexed), g_bsz)
        ]
        ...
        uni_batches = [
            uni_reindexed[i : i + g_bsz] for i in range(0, len(uni_reindexed), g_bsz)
        ]
    ...
    def __iter__(self) -> Iterator:
        ...
        assert (
            len(set(indices)) == len(self.modality_lengths) == len(self.dataset)
        ), "Oops!"
