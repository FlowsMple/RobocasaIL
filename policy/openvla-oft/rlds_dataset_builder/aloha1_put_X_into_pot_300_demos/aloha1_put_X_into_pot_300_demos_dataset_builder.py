...
def _generate_examples(paths) -> Iterator[Tuple[str, Any]]:
    ...
    def _parse_example(episode_path):
        ...
        with h5py.File(episode_path, "r") as F:
            ...
            low_cam_images = F["/observations/images/cam_low"][
                ()
            ]  # Low third-person camera

        # Get language instruction
        # Assumes filepaths look like: "/PATH/TO/ALOHA/PREPROCESSED/DATASETS/<dataset_name>/train/episode_0.hdf5"
        raw_file_string = episode_path.split("/")[
            -3
        ]  # E.g., '/scr/moojink/data/aloha1_preprocessed/put_green_pepper_into_pot/train/episode_0.hdf5' -> put_green_pepper_into_pot
...
class aloha1_put_X_into_pot_300_demos(MultiThreadedDatasetBuilder):
    ...
    def _split_paths(self):
        ...
        return {
            "train": glob.glob(
                "/scr/moojink/data/aloha1_preprocessed/put_green_pepper_into_pot/train/*.hdf5"
            )
            + glob.glob(
                "/scr/moojink/data/aloha1_preprocessed/put_red_pepper_into_pot/train/*.hdf5"
            )
            + glob.glob(
                "/scr/moojink/data/aloha1_preprocessed/put_yellow_corn_into_pot/train/*.hdf5"
            ),
            "val": glob.glob(
                "/scr/moojink/data/aloha1_preprocessed/put_green_pepper_into_pot/val/*.hdf5"
            )
            + glob.glob(
                "/scr/moojink/data/aloha1_preprocessed/put_red_pepper_into_pot/val/*.hdf5"
            )
            + glob.glob(
                "/scr/moojink/data/aloha1_preprocessed/put_yellow_corn_into_pot/val/*.hdf5"
            ),
        }
