...
def eval_aloha(cfg: GenerateConfig) -> None:
    ...
    for episode_idx in tqdm.tqdm(range(cfg.num_rollouts_planned)):
        ...
        (
            ...
            cfg, env, task_description, server_endpoint, resize_size, log_file
        )
        ...
        log_message(
            f"# successes: {total_successes} ({total_successes / num_rollouts_completed * 100:.1f}%)",
            log_file,
        )
        log_message(
            f"Total model query time: {episode_stats['model_query_time']:.2f} sec",
            log_file,
        )
        log_message(
            f"Total episode elapsed time: {episode_stats['episode_duration']:.2f} sec",
            log_file,
        )

    # Calculate final success rate
    final_success_rate = (
        float(total_successes) / float(num_rollouts_completed)
