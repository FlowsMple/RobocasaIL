...
def randomly_split(
    full_qpos, full_qvel, full_effort, full_action, full_image_dict, percent_val
):
    ...
    print(
        f"Total # steps: {num_episodes_total}; using {num_episodes_val} ({percent_val:.2f}%) for val set"
    )
...
def save_new_hdf5(out_dataset_dir, data_dict, episode_idx):
    ...
    with h5py.File(
        ...
        for cam_name in camera_names:
            ...
            root[f"/observations/images/{cam_name}"][...] = data_dict["image_dict"][
                cam_name
            ]
        ...
        relative_actions[:-1] = (
            actions[1:] - actions[:-1]
        )  # Relative actions are the changes in joint pos
        relative_actions[-1] = relative_actions[
            -2
        ]  # Just copy the second-to-last action for the last action
...
def main(args):
    ...
    out_dataset_dir = os.path.join(
        args.out_base_dir, os.path.basename(args.dataset_path.rstrip("/"))
    )
    os.makedirs(out_dataset_dir, exist_ok=True)
    # Get list of filepaths of all episodes
    all_demo_paths = glob.glob(
        os.path.join(args.dataset_path, "*.hdf5")
    )  # List of HDF5 filepaths
    all_demo_paths.sort()
    # Create a list of episode indices
    ...
    print(
        f"Total # episodes: {num_episodes_total}; using {num_episodes_val} ({args.percent_val:.2f}%) for val set"
    )
    num_episodes_train = num_episodes_total - num_episodes_val
