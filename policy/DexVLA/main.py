...
def compare_lora_weights():
    ...
    ema_ckpt = safe_open(
        os.path.join(path, "ema", "adapter_model.safetensors"), framework="pt"
    )
...
def compare_zero_weights(tag="global_step30000"):
    ...
    ema_ckpt = torch.load(
        os.path.join(
            path, "ema", tag, "bf16_zero_pp_rank_6_mp_rank_00_optim_states.pt"
        ),
        map_location=torch.device("cpu"),
    )["optimizer_state_dict"]
    print(ckpt.keys())
...
def compare_ema_weights():
    ckpt = torch.load(
        os.path.join(path, "non_lora_trainables.bin"), map_location=torch.device("cpu")
    )
    ema_ckpt = torch.load(
        os.path.join(path, "ema_weights_trainable.pth"),
        map_location=torch.device("cpu"),
    )
    ...
    for k in ema_ckpt.keys():
        ...
        if "policy_head" in k:
            ...
            for i, j in zip(false_indices[0], false_indices[1]):
                print(
                    ckpt[k].shape,
                    ckpt[k][i][j].to(ema_ckpt[k].dtype).item(),
                    ema_ckpt[k][i][j].item(),
                )
            break
        if k in ckpt.keys():
            print(
                ...
                ckpt[k].dtype,
                ema_ckpt[k].dtype,
                torch.equal(ckpt[k].to(ema_ckpt[k].dtype), ema_ckpt[k]),
            )
        else:
...
def debug():
    ...
    for k in ema_state_dict.keys():
        print(
            ...
            torch.equal(state_dict[k], ema_state_dict[k]),
        )
