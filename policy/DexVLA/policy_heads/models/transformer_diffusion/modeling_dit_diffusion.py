...
class TimestepEmbedder(nn.Module):
    ...
    def timestep_embedding(t, dim, max_period=10000):
        ...
        freqs = torch.exp(
            -math.log(max_period)
            * torch.arange(start=0, end=half, dtype=torch.bfloat16)
            / half
        ).to(device=t.device)
...
class DiT(PreTrainedModel):
    ...
    def __init__(
        ...
        self.output_dim = (
            config.output_dim * 2 if config.learn_sigma else config.output_dim
        )
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, config.prediction_horizon, config.n_emb)
        )

        self.blocks = nn.ModuleList(
            [
                DiTBlock(config.n_emb, config.num_heads, mlp_ratio=config.mlp_ratio)
    ...
    def forward(self, actions, hidden_states, states, is_pad, external_obs_cond=None):
        ...
        if actions is not None:  # training time
            ...
            noise = torch.randn(
                [num_noise_samples] + list(actions.shape),
            ...
            if (
                self.external_vision_encoder == "resnet"
                and external_obs_cond is not None
            ):
                hidden_states = torch.cat(
                    [hidden_states, external_obs_cond.unsqueeze(1)], dim=2
                )
    ...
    def model_forward(self, x, t, global_cond, states):
        ...
        global_cond = (
            torch.cat([global_cond, states], dim=-1)
