...
class DexVLADataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        attention_mask = [
            torch.flip(instance["attention_mask"].squeeze(0), dims=[0])
            for instance in instances
        ]
        labels = [
            torch.flip(instance["labels"].squeeze(0), dims=[0])
            for instance in instances
        ]
        raw_images = torch.stack([instances["raw_images"] for instances in instances])
        if self.video:
            video_grid_thw = torch.stack(
                [instances["video_grid_thw"] for instances in instances]
            )
            pixel_values_videos = torch.stack(
                [instances["pixel_values_videos"] for instances in instances]
            )
            pixel_values = None
            image_grid_thw = None
        else:
            image_grid_thw = torch.stack(
                [instances["image_grid_thw"] for instances in instances]
            )
            pixel_values = torch.stack(
                [instances["pixel_values"] for instances in instances]
            )
        ...
        b = input_ids.shape[0]
        if self.video:
            video_grid_thw = video_grid_thw.reshape(
                b * video_grid_thw.shape[1], video_grid_thw.shape[2]
            )
            pixel_values_videos = pixel_values_videos.reshape(
                b * pixel_values_videos.shape[1], pixel_values_videos.shape[2]
            )

        else:
            image_grid_thw = image_grid_thw.reshape(
                b * image_grid_thw.shape[1], image_grid_thw.shape[2]
            )
            pixel_values = pixel_values.reshape(
                b * pixel_values.shape[1], pixel_values.shape[2]
            )
        ...
        if not isinstance(instances[0]["action"], torch.Tensor):
            actions = torch.tensor(
                np.array([instance["action"] for instance in instances])
            )
            states = torch.tensor(
                np.array([instance["state"] for instance in instances])
            )
        else:
