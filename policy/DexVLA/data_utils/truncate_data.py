        ...
        with h5py.File(output_dataset_path, "w") as outfile:
            ...
            for key in infile.keys():
                if key != "observations" and key != "compress_len":
                    ...
                    out_data = outfile.create_dataset(
                        key, (TRUNCATE_LEN, data.shape[1])
                    )
                    out_data[:] = data

            data_compress_len = infile["compress_len"]
            out_data_compress_len = outfile.create_dataset(
                "compress_len", data_compress_len.shape
            )
            ...
            for key in obs_group.keys():
                if key != "images":
                    ...
                    out_data = out_obs_group.create_dataset(
                        key, (TRUNCATE_LEN, data.shape[1])
                    )
                    out_data[:] = data
            ...
            for cam_name in image_group.keys():
                ...
                out_data = out_image_group.create_dataset(
                    cam_name, (TRUNCATE_LEN, data.shape[1]), dtype="uint8"
                )
                out_data[:] = data
