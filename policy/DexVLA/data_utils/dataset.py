...
class EpisodicDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.rank0_print(
            f"########################Current Image Size is [{self.data_args.image_size_stable}]###################################"
        )
        self.rank0_print(
            f"{RED}policy class: {self.policy_class}; augument: {True}{RESET}"
        )
        a = self.__getitem__(0)  # initialize self.is_sim and self.transformations
        if len(self.camera_names) > 2:
            # self.rank0_print("%"*40)
            self.rank0_print(
                f"The robot is {RED} {self.robot} {RESET} | The camera views: {RED} {self.camera_names} {RESET} | The history length: {RED} {self.data_args.history_images_length} {RESET}"
            )
        self.is_sim = False
    ...
    def __getitem__(self, index):
        ...
        try:
            (
                ...
                raw_lang,
                reasoning,
            ) = self.load_from_h5(dataset_path, start_ts)
        ...
        if self.robot == "franka":
            assert (
                image_data.ndim == 4
            ), f"image_data's shape is {image_data.shape}, maybe the reason of adding historical images"
...
def load_data(
    ...
    dataset_path_list_list = [
        find_all_hdf5(dataset_dir, skip_mirrored_data, rank0_print=rank0_print)
    ...
    rank0_print(
        f"{RED}All images: {sum(all_episode_len)}, Trajectories: {len(all_episode_len)} {RESET}"
    )
    train_episode_len_l = [
    ...
    sampler_params = {
        "train": {
            ...
            "episode_len_l": train_episode_len_l,
            "sample_weights": sample_weights,
            "episode_first": config["data_args"].episode_first,
        },
        "eval": {
            "batch_size": batch_size_val,
            "episode_len_l": val_episode_len_l,
            "sample_weights": None,
            "episode_first": config["data_args"].episode_first,
        },
    }
