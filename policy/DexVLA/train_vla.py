...
class ActionHeadArguments:
    policy_head_type: str = field(
        default="dit_diffusion_policy"
...
class ModelArguments:
    ...
    only_using_input_embeddings: bool = field(
        default=False
    )  # using only input embeddings
    ...
    pretrain_dit_path: Optional[str] = field(
        default=None
    )  # path to pretrained dit weights
...
class DataArguments:
    ...
    episode_first: bool = (
        True  # batchsampler will samples episode index first and then samples timesteps
    )
...
def parse_param():
    ...
    config = AutoConfig.from_pretrained(
        model_args.model_name_or_path, **asdict(action_head_args)
    )
    ...
    if action_head_args.policy_head_type == "dit_diffusion_policy":
        ...
        raise NotImplementedError(
            f"Unsupported policy head type {action_head_args.policy_head_type}"
        )
    # for k,v in asdict(action_head_args).items():
    #     setattr(config, k, v)
    setattr(
        config.policy_head_config, "input_dim", asdict(action_head_args)["action_dim"]
    )
    setattr(
        config.policy_head_config, "state_dim", asdict(action_head_args)["state_dim"]
    )
    ...
    if model_args.is_tinyvla:
        rank0_print(
            f"{RED} This is TinyVLA, Please Check Both Using_film and Using_xattn equals False:Using_film {model_args.using_film}|Using_xattn {model_args.using_xattn} {RESET}"
        )
        time.sleep(1)
    return (
        model_args,
...
def train_bc(
    ...
    compute_dtype = (
        torch.float16
        if training_args.fp16
        else (torch.bfloat16 if config["training_args"].bf16 else torch.float32)
    )
    ...
    if "paligemma" in config["model_args"].model_name_or_path.lower():
        data_collator = PaliGemmaVLADataCollatorForSupervisedDataset(
            multimodal_processor=processor, computed_type=compute_dtype
        )

    else:
        data_collator = DexVLADataCollatorForSupervisedDataset(
    ...
    if config["training_args"].lora_enable:
        ...
        if (
            config["training_args"].local_rank == 0
            or config["training_args"].local_rank == -1
        ):
            model.config.save_pretrained(config["training_args"].output_dir)
            model.save_pretrained(
                config["training_args"].output_dir, state_dict=state_dict
            )
            torch.save(
                non_lora_state_dict,
                os.path.join(
                    config["training_args"].output_dir, "non_lora_trainables.bin"
                ),
            )
    else:
        ml_utils.safe_save_model_for_hf_trainer(
...
def main(all_config=None, model_config=None):
    ...
    if "paligemma" in all_config["model_args"].model_name_or_path.lower():
        ...
        rank0_print(
            f"{RED} PaliGemma using default and constant Image size{image_size}, omitting SuperParamter:[image_size_stable, image_size_wrist] {RESET}"
        )

        vla_process = PaliGemmaVLAProcess(
            tokenizer=tokenizer,
    ...
    stats_path = os.path.join(
        all_config["training_args"].output_dir, f"dataset_stats.pkl"
    )
...
if __name__ == "__main__":
    ...
    config_dict = {
        k: asdict(v) if not isinstance(v, dict) else v for k, v in config.items()
    }

    ckpt = os.path.join(
        config["training_args"].output_dir,
        f"checkpoint-{config['training_args'].save_steps}",
    )
