...
def label_clean_tables(DATA_DIR, model, processor, task):
    ...
    for episode in tqdm(episodes[:10]):
        ...
        key_frames, idxs = filter_images_by_subtask(
            image_dict, subtask, OUTPUT_DIR, episode.split(".")[0]
        )
            ...
            messages[0]["content"].append(
                {
                    "type": "image",
                    "image": os.path.join(
                        OUTPUT_DIR, f'{episode.split(".")[0]}_{i}.png'
                    ),
                }
            )
        messages[0]["content"].append({"type": "text", "text": f""})
        messages[0]["content"][-1]["text"] = prompt.replace(
            "[<objects>]", f"[{(','.join(objs))}]"
        )
...
if __name__ == "__main__":
    model, processor = load_model(
        "/home/jovyan/tzb/wjj/model_param/Qwen2-VL-7B-Instruct/"
    )
    tasks = [
    ...
    for task in tasks:
        label_clean_tables(
            DATA_DIR=DATA_DIR, task=task, model=model, processor=processor
        )
