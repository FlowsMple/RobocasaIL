...
class qwen2_vla_policy:
    ...
    def load_policy(self, policy_config):
        ...
        model_base = (
            policy_config["model_base"] if policy_config["enable_lora"] else None
        )
        ...
        (
            ...
            model_path=model_path, model_base=model_base, policy_config=policy_config
        )
    ...
    def process_batch_to_qwen2_vla(self, curr_image, robo_state, raw_lang):
        ...
        image_data = torch.chunk(
            curr_image, curr_image.shape[0], dim=0
        )  # top, left_wrist, right_wrist
        image_list = []
        for i, each in enumerate(image_data):
            ele = {}
            each = Image.fromarray(
                each.cpu().squeeze(0).permute(1, 2, 0).numpy().astype(np.uint8)
            )
            ele["image"] = each
...
class DexVLA:
    ...
    def get_action(self, obs):
        ...
        with torch.inference_mode():
            cur_state_np_raw, robot_state, traj_rgb_np = get_obs(
                obs, stats, camera_views=policy_config["camera_views"]
            )
            # 转换数据格式
            ...
            all_actions, outputs = policy.policy.evaluate(
                **batch, is_eval=True, tokenizer=policy.tokenizer, raw_images=curr_image
            )
