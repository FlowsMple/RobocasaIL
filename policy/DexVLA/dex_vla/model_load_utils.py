...
def load_model(config=None, qwen2_vla_config=None, rank0_print=print, tokenizer=None):
    ...
    if training_args.load_pretrain:  # loading pretrained weights
        ...
        assert (
            config["model_args"].model_pretrain is not ""
        ), "load pretrain weights need set the model_pretrain in DataArguments!!!!"
        ...
        rank0_print(
            f"{RED}Loading pretrained <<{config['model_args'].model_pretrain}>> from base models...{RESET}"
        )
        ...
        rank0_print(
            f"Loading pretrained additional <<{model_path}/non_lora_trainables.bin>> weights..."
        )
    ...
    if (
        model_args.load_pretrain_dit
        and not config["training_args"].resume_from_checkpoint
    ):
        assert (
            model_args.pretrain_dit_path is not None
        ), "please specify a pretrained dit path when setting load_pretrain_dit==True"
        rank0_print(
            f"{RED}>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Loading pretrained dit weights...<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<{RESET}"
        )
        pretrain_dit_weights = torch.load(
            model_args.pretrain_dit_path, map_location="cpu"
        )
        if (not model_args.Using_EMA_Pretrain_DiT) or (
            "use_constant_1" in model_args.pretrain_dit_path
        ):
        ...
        keys_to_del_dit = []
        pretrain_dit_weights = {
            k[7:] if k.startswith("policy.") else k: v
            for k, v in pretrain_dit_weights.items()
        }
        ...
        if model_args.external_vision_encoder != "None":
            model.external_vision_encoder_model.load_state_dict(
                pretrain_dit_weights, strict=False
            )
    ...
    if "paligemma" in config["model_args"].model_name_or_path.lower():
        ...
        model.config.freeze_vision_tower = (
            model_args.freeze_vision_tower
        ) = training_args.freeze_vision_tower
    ...
    if training_args.lora_enable:
        ...
        lora_config = LoraConfig(
            ...
            target_modules=find_all_linear_names(
                model, rank0_print, training_args.lora_module
            ),
        ...
        model = get_peft_model(
            model, lora_config
        )  # !!!only set lora weights to requires_grad True!!!
        rank0_print(model)
        model.print_trainable_parameters()
    elif training_args.load_pretrain:
        rank0_print(
            "Already loaded pretrained weights which is based on lora, skipping LoRA initialize..."
        )
    ...
    rank0_print(
        f"{RED}Lora parameters/trainalbe parameters/all parameters:{lora_para/1000000}M/{train_para/1000000}M/{(all_para-lora_para)/1000000}M{RESET}"
    )
...
def load_merge_lora_weights(
    model_path=None, model_base=None, kwargs=None, pretrain_dit_path=None
):
    path = model_path.split("/")[0:-1]
    ...
    print(
        "Loading additional QWen2-VLA weights expecially non-lora part(diffusion head)..."
    )
    if os.path.exists(os.path.join(model_path, "ema_adapter")):
        non_lora_trainables = torch.load(
            os.path.join(model_path, "ema_adapter", "ema_non_lora_trainables.bin"),
        )
    ...
    if pretrain_dit_path is not None:
        ...
        pretrain_dit_weights = torch.load(pretrain_dit_path, map_location="cpu")[
            "nets"
        ]["ema"]
        keys_to_del_dit = []
        pretrain_dit_weights = {
    ...
    ) and os.path.exists(os.path.join(model_path, "ema_adapter")):
        print("Loading EMA LoRA weights...")
        model = PeftModel.from_pretrained(
            model, os.path.join(model_path, "ema_adapter")
        )
        ...
        if "lora" in model_path.lower() and model_base is not None:
            if policy_config["pretrain_path"] is not None:
                ...
                if not os.path.exists(
                    os.path.join(
                        policy_config["pretrain_path"], "pretrain_merge_weights"
                    )
                ):
                    print("merging pretrained weights.......")
                    model, tokenizer = load_merge_lora_weights(
                        model_path=policy_config["pretrain_path"],
                ...
                model, tokenizer = load_merge_lora_weights(
                    ...
                    model_base=model_base,
                    kwargs=kwargs,
                    pretrain_dit_path=policy_config["pretrain_dit_path"],
                )
            ...
            if policy_config["save_model"]:
                ...
                model.save_pretrained(os.path.join(model_path, "merge_weights"))
                ...
                torch.save(
                    head_param,
                    os.path.join(model_path, "merge_weights/head_params.bin"),
                )
                multi_modal_processor = AutoProcessor.from_pretrained(
                    model_path, use_fast=False
                )
                multi_modal_processor.save_pretrained(
