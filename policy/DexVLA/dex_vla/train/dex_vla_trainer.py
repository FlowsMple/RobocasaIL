...
class CustomBatchSampler(Sampler):
    def __init__(
        ...
        self.sum_dataset_len_l = np.cumsum(
            ...
            for _ in range(self.batch_size):
                if self.episode_first:
                    episode_idx = np.random.choice(
                        len(self.episode_len_l), p=self.sample_probs
                    )
                    step_idx = np.random.randint(
                        self.sum_dataset_len_l[episode_idx],
                        self.sum_dataset_len_l[episode_idx + 1],
                    )
                else:
...
class DexVLATrainer(Trainer):
    def __init__(self, sampler_params, prefetch_factor=0, *args, **kwargs):
        ...
        self.lang_type = (
            "model"
            if "phi" in kwargs["model"].config.architectures[0].lower()
            else "gpt_neox"
        )
        ...
        if self.using_ema:
            if self.local_rank == 0:
                print(
                    ">>>>>>>>>>>>>>>>>>>>>>>>>>Model weights is updated by EMA.<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
                )
            ...
            if self.resume_from_checkpoint:
                if self.local_rank == 0:
                    ...
                    ckpt_dirs = glob.glob(
                        os.path.join(kwargs["args"].output_dir, "checkpoint-*")
                    )
                    ckpt_dirs = sorted(ckpt_dirs, key=lambda x: int(x.split("-")[-1]))
                    ema_state_dict = torch.load(
                        os.path.join(
                            kwargs["args"].output_dir, ckpt_dirs[-1], "ema_weights.pth"
                        ),
                        map_location="cpu",
                    )
        ...
        if self.optimizer is None:
            ...
            if self.args.non_lora_lr is not None:
                ...
                for name, module in opt_model.named_parameters():
                    ...
                    if (
                        "policy_head" not in name
                        and "layers" in name
                        and "vision" not in name
                        and self.lang_type in name
                    ):  # gptneoxl LLM的参数
                        ...
                        key in name for key in non_lora_modules
                    ):  # vision adapter、action head的参数
                        # non_lora_parameters.append(name)
                        non_lora_parameters.append(name)
                ...
                optimizer_grouped_parameters = [
                    {
                        "params": [
                            ...
                            if (
                                ...
                                and n not in non_lora_parameters
                                and p.requires_grad
                            )  # lora and decay
                        ],
                ...
                assert (
                    len(optimizer_grouped_parameters[1]["params"]) == 0
                ), f"{optimizer_grouped_parameters[1]['params']} should be empty!!!!!"
            else:
                optimizer_grouped_parameters = [
                    {
                        "params": [
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            self._maybe_log_save_evaluate(
                tr_loss, model, trial, epoch, ignore_keys_for_eval, all_loss=custom_loss
            )
            ###############################################################################################################################################
        ...
        if (
            ...
            for k, v in custom_loss.items():
                if k == "loss":
                    continue
                logs[k] = round(
                    v / (self.state.global_step - self._globalstep_last_logged), 4
                )
            ################################################################################################################
        ...
        if self.control.should_save:
            ...
            if self.using_ema:
                ...
                if self.local_rank == 0:
                    ...
                    print(
                        f"-----------------------------Saving EMA Weights on {self.local_rank}-----------------------------"
                    )
                    torch.save(
                        ema_state_dict, os.path.join(output_dir, "ema_weights.pth")
                    )
        ...
        # Load adapters following PR # 24096
            ...
            if hasattr(model, "active_adapter") and hasattr(model, "load_adapter"):
                if os.path.exists(resume_from_checkpoint):
                    model.load_adapter(
                        resume_from_checkpoint, model.active_adapter, is_trainable=True
                    )
                else:
                    logger.warning(
