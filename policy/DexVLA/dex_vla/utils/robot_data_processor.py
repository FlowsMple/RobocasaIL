...
class DexVLAProcess:
    ...
    def qwen2_image_preprocess(self, each, camera_name):
        ...
        each = Image.fromarray(
            each.squeeze(0).permute(1, 2, 0).numpy().astype(np.uint8)
        )
        ele["image"] = each
    ...
    def forward_process(self, sample, use_reasoning=True):
        ...
        data_dict = dict(messages=messages, images=None)
        ...
        for i, each in enumerate(image_data):
            if each.ndim == 4:
                ...
                for temp in each.squeeze(0):
                    img_pil.append(
                        self.qwen2_image_preprocess(temp, self.camera_names[i])
                    )
        ...
        model_inputs["input_ids"] = torch.cat(
            (model_inputs["input_ids"], output_text["input_ids"]), dim=-1
        )
