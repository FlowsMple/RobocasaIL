...
class DexVLA(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.using_first_layer_hidden_states = getattr(
            config, "using_first_layer_hidden_states", False
        )
        self.llm_loss_weight = config.llm_loss_weight
        self.using_state = getattr(config, "using_state", False)

        self.external_vision_encoder = getattr(
            config, "external_vision_encoder", "None"
        )

        if isinstance(config.policy_head_config, dict):
            config.policy_head_config = AutoConfig.for_model(
                **config.policy_head_config
            )
        ...
        if self.using_state:
            self.state_proj = nn.Linear(
                config.policy_head_config.state_dim, config.hidden_size
            )
        ...
        self.reasoning_action_proj = ActionProjector(
            config.hidden_size, config.hidden_size
        )

        if self.using_film:
            self.reasoning_film = FiLM(
                feature_dim=config.hidden_size, condition_dim=config.hidden_size
            )
    ...
    def forward(
        ...
        if not is_eval:
            ...
            if self.using_state:
                attn_mask_state_embedding = torch.ones(
                    (attention_mask.shape[0], 1),
                    dtype=torch.bool,
                    device=attention_mask.device,
                )
                attention_mask = torch.cat(
                    [attention_mask, attn_mask_state_embedding], dim=-1
                )
                ...
                start_idxs = torch.argmax(
                    torch.flip(mask, dims=[1]), dim=1
                )  # shape: [4, 100]
        ...
        action_hidden_states = self.fusion_input_reasoning(
            labels=labels,
            input_ids=input_ids,
        ...
        ret = self.policy_head(
            ...
            states=states,
            is_pad=is_pad,
            external_obs_cond=external_obs_cond,
        )
    ...
    def fusion_input_reasoning(
        ...
        for i in range(indexs.shape[0]):
            ...
            input_embeddings_first_layer.append(
                torch.mean(prev_layer_hidden_states[0][i, start:end, :], dim=0)
            )
        ...
        if self.using_first_layer_hidden_states:
            action_hidden_states = (
                action_hidden_states + input_embeddings_first_layer.unsqueeze(1)
            )
    ...
    def xattn_forward(self, labels, input_ids, hidden_states):
        ...
        for i in range(indexs.shape[0]):
            ...
            start = sum(temp.int())
            input_embeddings.append(
                torch.flip(hidden_states[i, start:end, :], dims=[1])
            )
            reasoning_embeddings.append(torch.flip(hidden_states[i, end:, :], dims=[1]))
        padd_value = 0

        input_embeddings = torch.nn.utils.rnn.pad_sequence(
            input_embeddings, batch_first=True, padding_value=padd_value
        )
        reasoning_embeddings = torch.nn.utils.rnn.pad_sequence(
            reasoning_embeddings, batch_first=True, padding_value=padd_value
        )
        ...
        x_attn_mask = torch.matmul(
            r_mask.unsqueeze(-1), i_mask.unsqueeze(-1).transpose(-1, -2)
        )
        x_attn_mask = x_attn_mask.masked_fill(x_attn_mask == 0, float(-50)).masked_fill(
            x_attn_mask == 1, float(0.0)
        )
        ...
        action_hidden_states = self.xattn(
            encoder_hidden_states=input_embeddings,
            hidden_states=reasoning_embeddings,
            encoder_attention_mask=x_attn_mask,
        )[0]
    ...
    def evaluate(
        ...
        ground_truth_reasoning_embed = self.model.embed_tokens(
            torch.tensor(new_output_ids).to("cuda")
        ).unsqueeze(0)
        ...
        action = self.policy_head(
            actions,
            action_hidden_states,
            states.to(all_hidden_states.dtype),
            is_pad,
            external_obs_cond=external_obs_cond,
        )
        return action, outputs_text

    def evaluate_tinyvla(
        self,
        input_ids: torch.LongTensor = None,
        actions=None,
        states=None,
        ...
        input_ids = input_ids.to("cuda")
        with torch.inference_mode():
            all_hidden_states = self.forward(
                input_ids,
                pixel_values=pixel_values,
                attention_mask=attention_mask,
                image_grid_thw=image_grid_thw,
