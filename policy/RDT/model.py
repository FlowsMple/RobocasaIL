...
class RDT:
    ...
    # encoding language
    def set_language_instruction(
        self, language_instruction, save_dir=None, task_name=None
    ):
        ...
        if os.path.isfile(language_instruction):
            ...
            print(
                f"Running with instruction: \"{lang_dict['instruction']}\" from \"{lang_dict['name']}\""
            )
    ...
    def get_action(self, img_arr=None, state=None):
        ...
        with torch.inference_mode():
            action_buffer = inference_fn(
                self.config, self.policy, self.lang_embeddings, self.observation_window
            ).copy()
...
def inference_fn(config, policy, lang_embeddings, observation_window):
    ...
    while True:
        ...
        actions = (
            policy.step(proprio=proprio, images=images, text_embeds=lang_embeddings)
            .squeeze(0)
            .cpu()
            .numpy()
        )
