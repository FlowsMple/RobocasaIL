...
def serialize_example(
    ...
    feature = {
        ...
        "cam_high": _bytes_feature(
            tf.io.serialize_tensor(
                tf.convert_to_tensor(cam_high.tobytes(), dtype=tf.string)
            )
        ),
        "cam_left_wrist": _bytes_feature(
            tf.io.serialize_tensor(
                tf.convert_to_tensor(cam_left_wrist.tobytes(), dtype=tf.string)
            )
        ),
        "cam_right_wrist": _bytes_feature(
            tf.io.serialize_tensor(
                tf.convert_to_tensor(cam_right_wrist.tobytes(), dtype=tf.string)
            )
        ),
...
def process_hdf5_file(args):
    ...
    output_dir = os.path.join(
        out_dir, os.path.relpath(os.path.dirname(filepath), root_dir)
    )
    ...
    try:
        with h5py.File(filepath, "r") as f, tf.io.TFRecordWriter(
            tfrecord_path
        ) as writer:
...
def write_tfrecords(root_dir, out_dir):
    ...
    for root, dirs, files in os.walk(root_dir):
        if os.path.exists(os.path.join(root, "expanded_instruction_gpt-4-turbo.json")):
            ...
            shutil.copy(
                os.path.join(root, "expanded_instruction_gpt-4-turbo.json"), target_path
            )
