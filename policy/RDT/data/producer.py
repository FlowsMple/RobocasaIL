        ...
        try:
            ...
            with open(file_path, "wb") as file:
                np.savez(
                    ...
                    past_frames_0_time_mask=step_dict[
                        "past_frames_0_time_mask"
                    ].numpy(),
                    past_frames_1=step_dict["past_frames_1"].numpy(),
                    past_frames_1_time_mask=step_dict[
                        "past_frames_1_time_mask"
                    ].numpy(),
                    past_frames_2=step_dict["past_frames_2"].numpy(),
                    past_frames_2_time_mask=step_dict[
                        "past_frames_2_time_mask"
                    ].numpy(),
                    past_frames_3=step_dict["past_frames_3"].numpy(),
                    past_frames_3_time_mask=step_dict[
                        "past_frames_3_time_mask"
                    ].numpy(),
    ...
    for episode_steps in vla_dataset:
        for step in episode_steps:
            if fill_up and fill_chunk_idx < chunk_end_idx:
                ...
                if (
                    local_fill_chunk_idx % 10 == 0
                    or local_fill_chunk_idx == local_num_chunks - 1
                ) and fill_chunk_item_idx == 0:
                    print(
                        f"Worker {worker_id}: Filled up chunk {local_fill_chunk_idx+1}/{local_num_chunks}"
                    )
                ...
                if fill_chunk_idx == BUF_NUM_CHUNKS:
                    print(
                        f"Worker {worker_id}: Buffer filled up. Start replacing dirty samples..."
                    )
                ...
                while len(dirty_chunk_item_idxs) == 0:
                    ...
                    if time.time() - time_stmp > 2.0:
                        ...
                        print(
                            f"Worker {worker_id}: Dirty Ratio for Chunk {dirty_chunk_idx}: {dirty_ratio:.2f}"
                        )
                        time_stmp = time.time()
                ...
                if len(dirty_chunk_item_idxs) == 0:
                    ...
                    save_dirty_bit(dirty_chunk_dir, dirty_bit)
                    print(
                        f"Worker {worker_id}: Replaced dirty chunk {dirty_chunk_idx}."
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--clean_dirty",
        action="store_true",
        help="Whether to clean the dirty bits before replacing dirty samples. This option is ignored when `fill_up` is set.",
    )
    parser.add_argument(
        "--seed",
        type=int,
