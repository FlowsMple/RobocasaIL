...
def log_sample_res(
    ...
    with torch.autocast(device_type="cuda", dtype=torch.float16):
        ...
        for step, batch in enumerate(dataloader):
            ...
            expanded_state_elem_mask = (
                state_elem_mask.unsqueeze(1).tile((1, num_steps, 1)).float()
            )
            expanded_state_norm = (
                state_norm.unsqueeze(1).tile((1, num_steps, 1)).float()
            )
            ...
            mse_loss_per_entry = (loss * expanded_state_elem_mask).reshape(
                (batch_size, -1)
            ).sum(1) / expanded_state_elem_mask.reshape((batch_size, -1)).sum(1)
            ...
            if accelerator.is_main_process:
                for loss_suffix, losses in zip(
                    ["_sample_mse", "_sample_l2err"], [mse_losses, l2_losses]
                ):
            ...
            mse_loss = (
                loss * expanded_state_elem_mask
            ).sum() / expanded_state_elem_mask.sum()
            ...
            l2_loss = (
                l2_loss * expanded_state_elem_mask
            ).sum() / expanded_state_elem_mask.sum()
