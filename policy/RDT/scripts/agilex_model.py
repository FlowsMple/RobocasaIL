...
def create_model(args, **kwargs):
    ...
    AGILEX_STATE_INDICES = (
        [STATE_VEC_IDX_MAPPING[f"left_arm_joint_{i}_pos"] for i in range(left_arm_dim)]
        + [STATE_VEC_IDX_MAPPING["left_gripper_open"]]
        + [
            STATE_VEC_IDX_MAPPING[f"right_arm_joint_{i}_pos"]
            for i in range(right_arm_dim)
        ]
        + [STATE_VEC_IDX_MAPPING[f"right_gripper_open"]]
    )
    ...
    def _format_joint_to_state(self, joints):
        ...
        AGILEX_STATE_INDICES = (
            [
                ...
                for i in range(self.left_arm_dim)
            ]
            + [STATE_VEC_IDX_MAPPING["left_gripper_open"]]
            + [
                STATE_VEC_IDX_MAPPING[f"right_arm_joint_{i}_pos"]
                for i in range(self.right_arm_dim)
            ]
            + [STATE_VEC_IDX_MAPPING[f"right_gripper_open"]]
        )
