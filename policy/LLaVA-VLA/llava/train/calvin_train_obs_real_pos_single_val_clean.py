    ...
    if getattr(trainer.args, "tune_audio_adapter", False):
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith("checkpoint-"):
                ...
                torch.save(
                    weight_to_save, os.path.join(output_dir, f"mm_projector_audio.bin")
                )
        return
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            if "<image>" in rou and not "<audio>" in rou:
                round_len = len(
                    tokenizer_image_token(rou, tokenizer)
                )  # 统计长度时多了<s>, 但少了</s>, 所以不变
                instruction_len = (
                    len(tokenizer_image_token(parts[0], tokenizer)) - 2
                )  # 统计长度时多了<s>, 且': '在句尾时多了' ', 所以减2
                ...
                round_len = len(tokenizer_audio_token(rou, tokenizer))
                instruction_len = (
                    len(tokenizer_image_audio_token(parts[0], tokenizer)) - 2
                )
            else:
            ...
            if (
                i != 0 and not tokenizer.legacy and IS_TOKENIZER_GREATER_THAN_0_14
            ):  # USER正常tokenize为两个token，但其前有特殊字符，如</s>时会tokenize为1个token，所以减1
                round_len -= 1
                instruction_len -= 1
...
class LeRobotDataset_Llava(LeRobotDataset):
    ...
    def __getitem__(self, idx) -> dict:
        ...
        inputs = format_source_data_real(
            instruction, action, observation, conv, self.action_tokenizer
        )  # action 是list torch str 可以
        conversations = [inputs]
        input_ids = torch.stack(
            [tokenizer_image_token(inputs, self.tokenizer, return_tensors="pt")], dim=0
        )
...
def make_supervised_data_module_all(
    tokenizer: transformers.PreTrainedTokenizer,
    action_tokenizer: ActionTokenizer,
    data_args,
    do_eval: bool = True,
    ...
    print(
        "len(train_dataset):",
        len(train_dataset_all) if train_dataset_all is not None else 0,
    )
    print(
        "len(eval_dataset):",
        len(eval_dataset_all) if eval_dataset_all is not None else 0,
    )
    return dict(
...
def train(attn_implementation=None):
    ...
    robot_obs_lang = partial(
        encode_robot_obs_forpipper, statistics=data_args.action_stat
    )
    # print(training_args)
    ...
    if model_args.audio_tower is not None:
        ...
        model.config.tune_audio_adapter = (
            training_args.tune_audio_adapter
        ) = model_args.tune_audio_adapter
