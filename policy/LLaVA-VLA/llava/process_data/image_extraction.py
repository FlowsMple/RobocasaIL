...
def extract_images(hdf5_path, save_root_dir, task_name, task_config, episode_id=0):
    ...
    episode_folder = os.path.join(
        save_root_dir, task_name, task_config, f"episode{episode_id}"
    )
    os.makedirs(episode_folder, exist_ok=True)

    with h5py.File(hdf5_path, "r") as f:
        num_frames = len(
            f[f"observation/{cameras[0]}/rgb"]
        )  # Use left_camera frame count as reference
        for i in range(num_frames):
            ...
            for cam in cameras:
                ...
                if img is None:
                    print(
                        f"[Warning] episode{episode_id} frame {frame_idx} {cam} image decoding failed, skipping this frame"
                    )
                ...
                if len(images) == len(cameras):
                    ...
                    print(
                        f"[Warning] episode{episode_id} frame {frame_idx} missing some images, skipping combination"
                    )
...
def process_episode_files(data_root_dir, save_root_dir, task_config, task_name=None):
    ...
    episode_files = sorted(
        ...
        key=lambda x: int(
            re.search(r"episode(\d+)\.hdf5", os.path.basename(x)).group(1)
        ),
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process HDF5 episodes and extract images"
    )
    ...
    parser.add_argument(
        "--data_root_dir", type=str, required=True, help="HDF5 data directory"
    )
    parser.add_argument(
        "--save_root_dir",
        type=str,
        required=True,
        help="Root directory for saving images",
    )
    args = parser.parse_args()

    process_episode_files(
        args.data_root_dir, args.save_root_dir, args.task_config, args.task_name
    )
