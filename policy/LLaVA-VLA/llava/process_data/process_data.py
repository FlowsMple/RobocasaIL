...
def generate_llm_dataset(
    ...
    episode_files = sorted(
        ...
        if re.search(r"episode(\d+)\.hdf5", os.path.basename(x))
        else float("inf"),
    )
    ...
    for file_path in episode_files:
        ...
        if not match:
            print(
                f"Unrecognized filename format, skipping: {os.path.basename(file_path)}"
            )
            continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate LLM-format dataset from HDF5 and images"
    )
    ...
    parser.add_argument(
        "--instruction_dir", type=str, required=True, help="Instruction JSON directory"
    )
    parser.add_argument(
        "--image_root_path",
        type=str,
        required=True,
        help="Image root path (absolute path)",
    )
    parser.add_argument(
        "--save_root_dir",
        type=str,
        required=True,
        help="Root directory for saving generated JSON files",
    )
    parser.add_argument(
        "--future_step_num", type=int, default=5, help="Number of future frame steps"
    )
