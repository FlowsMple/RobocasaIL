...
def collect_qpos_from_tasks(root_dir, task_list, task_config, output_yaml_path):
    ...
    assert task_config in [
        "demo_clean",
        "demo_randomized",
    ], "task_config must be 'demo_clean' or 'demo_randomized'"
    ...
    if not all_qpos_data:
        raise ValueError(
            f"No valid 'joint_action/vector' data found for tasks: {task_list}"
        )
    ...
    yaml_data = {
        "robot_obs": {"mean": mean_values, "std": std_values},
    ...
    for task_name in os.listdir(root_data_dir):
        ...
        if os.path.isdir(task_path):
            ...
            try:
                ...
                print(
                    f"? An unexpected error occurred while processing task '{task_name}': {e}"
                )
