...
def eval_model_with_env(
    ...
    if action_playback:
        ...
        num_test_steps = 3 * traj_len

    gt_action_joints_across_time = []
    pred_action_joints_across_time = []
    # flush the action queue upon rolling out
    if not action_playback:
    ...
    for i in range(num_test_steps):
        ...
        gt_action = []
        if i < traj_len - 1:
            gt_action.extend(states[i + 1, 5:12])
            gt_action.extend(actions[i, 6:11])
            gt_action.append(actions[i, -1])
        else:
            gt_action.extend(states[-1, 5:12])
            gt_action.extend(actions[-1, 6:11])
            gt_action.append(actions[-1, -1])
        gt_action = np.array(gt_action, dtype=np.float32)
        ...
        if action_playback:
            ...
            if count % policy_skip == 0:
                ...
                policy_obs["observation.state"].append(obs["robot0_gripper_qpos"][0])
                policy_obs["observation.state"] = torch.tensor(
                    policy_obs["observation.state"], device=device, dtype=torch.float32
                ).unsqueeze(0)
                policy_obs["task"] = [task_name]
                action_policy = (
                    policy.select_action(policy_obs).flatten().cpu().numpy().tolist()
                )
        ...
        pred_action_joints_across_time.append(action)
    ...
    fig, axes = plt.subplots(
        nrows=num_of_joints, ncols=1, figsize=(8, 4 * num_of_joints)
    )
    ...
    plt.legend()
    fig_name = f"traj_{time.strftime('%Y%m%d_%H%M%S')}.jpg"
...
def eval_model(args):
    # if args.action_playback:
    #     policy = None
    # else:
    #     # load model
    #     # ckpt_torch_dir = "lerobot/pi0"
    #     ckpt_torch_dir = "/data/ceph_hdd/main/dev/zim.gong/openpi/checkpoints/pi0_pnp/pi0_pytorch_29999"

    #     cfg = PreTrainedConfig.from_pretrained(ckpt_torch_dir)
    #     cfg.pretrained_path = ckpt_torch_dir
    #     dataset_meta = pickle.load(
    #         open(
    #             "/data/ceph_hdd/main/datasets/lerobot/robocasa/PnPCounterToStoveMG/meta/meta.pkl",
    #             "rb",
    #         )
    #     )
    ...
    if args.style_id is not None:
        demos = [
            demo
            for demo in demos
            if json.loads(f["data"][demo].attrs["ep_meta"])["style_id"] == args.style_id
        ]
    ...
    for ind in range(len(demos)):
        ...
        eval_model_with_env(
            ...
            action_playback=args.action_playback,
        )
...
def get_eval_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="(optional) filter demonstrations by style id",
    )
