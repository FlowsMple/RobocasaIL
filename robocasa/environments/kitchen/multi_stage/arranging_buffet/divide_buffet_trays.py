...
class DivideBuffetTrays(Kitchen):
    ...
    def _setup_kitchen_references(self):
        ...
        self.dining_counter = self.register_fixture_ref("dining_counter", dict(
            id=FixtureType.DINING_COUNTER, ref=self.stool1)
        )
    ...
    def _check_success(self):
        ...
        for tray in tray_names:
            has_meats = all(
                OU.check_obj_in_receptacle(self, meat, tray) for meat in ["meat1", "meat2"]
            )
            has_vegs = all(
                OU.check_obj_in_receptacle(self, veg, tray) for veg in ["vegetable1", "vegetable2"]
            )
            if has_meats:
                meat_tray = tray
            if has_vegs:
        ...
        meat_tray_vegs = any(
            OU.check_obj_in_receptacle(self, veg, meat_tray) for veg in ["vegetable1", "vegetable2"]
        )
        veg_tray_meats = any(
            OU.check_obj_in_receptacle(self, meat, veg_tray) for meat in ["meat1", "meat2"]
        )

        gripper_far = (
            OU.gripper_obj_far(self, obj_name="vegetable1") and
            OU.gripper_obj_far(self, obj_name="vegetable2") and
            OU.gripper_obj_far(self, obj_name="meat1") and
            OU.gripper_obj_far(self, obj_name="meat2") and
            OU.gripper_obj_far(self, obj_name="tray1_container") and
            OU.gripper_obj_far(self, obj_name="tray2_container")
        )
